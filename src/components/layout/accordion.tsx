...
import { ButtonDelete } from "../element/button";

export const AccordionLayout = (props: { className?: string; tabs: { title: string, content: any, deleteHandler?: any; }[] }) => {
    ...
    return (
        <div className={` ${props.className} w-full shadow  bg-[#00000011] rounded-xl p-2`}>
            {props.tabs.map((tab, _index) => (
                <div key={_index} className={_index === props.tabs.length - 1 ? '' : 'mb-4  '}>
                    <div className={getClass(_index)} >
                        ...
                        <div className=" w-[20px] mr-[30px]">
                            {tab.deleteHandler && <ButtonDelete clickHandler={tab.deleteHandler} />}
                        </div>
                    </div>
                    <Transition
